...
public class TableGRP extends JTable {
            ...
            public Component getTableCellRendererComponent(JTable jtable, Object o, boolean selected, boolean bln1, int i, int i1) {
                ...
                if (i1 != columnCount - 1) {
                    ...
                    if (jtable.getValueAt(i, 0) != null) {
                        Object idMValue = jtable.getValueAt(i, 0);
                        if (idMValue instanceof Integer) {
                            id = (int) idMValue;
                        } else if (idMValue instanceof String) {
                            try {
                                id = Integer.parseInt((String) idMValue);
                            } catch (NumberFormatException e) {
                                System.out.println("here comes the error");
                            }
                        }
                    }
                    ActionPanelGRP cell = new ActionPanelGRP(id);
                    return cell;
                }
            }
        });
        ...
        public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
            if (column == getColumnCount() - 1) {
                ...
                if (table.getValueAt(row, 0) != null) {
                    Object idMValue = table.getValueAt(row, 0);
                    if (idMValue instanceof Integer) {
                        id = (int) idMValue;
                    } else if (idMValue instanceof String) {
                        try {
